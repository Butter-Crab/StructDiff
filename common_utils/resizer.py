...
class Resizer(nn.Module):
    """
    Lightweight differentiable resizer used for low-frequency reference guidance.

    Adapted from https://github.com/assafshocher/resizer.
    """
    ...
    def forward(self, in_tensor):
        ...
        for dim, field_of_view, weights in zip(self.sorted_dims, self.field_of_view, self.weights):
            x = torch.transpose(x, dim, 0)
            x = torch.sum(x[field_of_view] * weights, dim=0)
            x = torch.transpose(x, dim, 0)
